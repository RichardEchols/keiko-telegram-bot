...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Check for natural language commands (like Brock)
    ...
    if blocked:
        await update.message.reply_text(
            f"🚫 That command is blocked for safety.\n"
            f"Blocked pattern: `{blocked}`"
        )
        logger.warning(f"Blocked command: {sanitize_for_logging(text)[:50]}")
        return

    # Check for confirmation required
    confirm_pattern = needs_confirmation(text)
    if confirm_pattern and user_id not in pending_confirmations:
        pending_confirmations[user_id] = text
        await update.message.reply_text(
            f"⚠️ This action requires confirmation.\n"
            f"Pattern detected: `{confirm_pattern}`\n\n"
    ...
    # Send immediate acknowledgment with confidence prefix
...
async def handle_media(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if update.message.voice:
        await update.message.reply_text(f"{BOT_EMOJI} 🎤 Processing voice message...")
        try:
            # Download voice file
            ...
            # Use Claude to transcribe (it can handle audio description)
            result, success = await execute_claude(
                f"I received a voice message from Richard. The audio file is at {voice_path}. "
                f"Please listen to it and respond to whatever he said. If you can't access the audio, "
                f"let him know and ask him to type his request."
            )
            ...
            if success:
                ...
                await update.message.reply_text(
                    f"{BOT_EMOJI} Couldn't process voice message. Please type your request."
                )
    ...
    if update.message.photo:
        await update.message.reply_text(f"{BOT_EMOJI} On it - looking at the image...")
        try:
...
async def post_init(application: Application) -> None:
    ...
    # Start heartbeat scheduler in background
    async def send_callback(text):
        await send_to_richard(application.bot, text)

    asyncio.create_task(start_heartbeat_scheduler(send_callback))
    logger.info("Heartbeat scheduler started")

    # Start learning loop in background
    asyncio.create_task(start_learning_loop(send_callback))
    logger.info("Learning loop started")

    # Start reminder scheduler in background
    asyncio.create_task(start_reminder_scheduler(send_callback))
    logger.info("Reminder scheduler started")

    # Start monitoring loop in background
    asyncio.create_task(start_monitoring_loop(send_callback))
    ...
    asyncio.create_task(start_health_monitor(send_callback))
