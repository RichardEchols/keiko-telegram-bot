...
from config import (
    ...
    NIGHTLY_WORK_HOUR, NIGHTLY_WORK_MINUTE,
    QUIET_HOURS_START, QUIET_HOURS_END,
    BOT_EMOJI,
)
from context import _atomic_json_write
            ...
            if _is_active(5) or _is_quiet():
                ...
                for task in tasks[:2]:
                    ...
                    if ok:
                        ...
                        executed += 1
                if executed:
                    await send_cb(f"{BOT_EMOJI} Completed {executed} background task(s)")
        except Exception as e:
...
def _save_reminders(reminders: List[Dict]):
    _atomic_json_write(REMINDERS_FILE, reminders)
...
async def _temp_cleanup_loop(send_cb):
    """Every 6 hours: clean temp files older than 24h."""
    while True:
        try:
                ...
                for f in temp_dir.iterdir():
                    if f.is_file():
                        ...
                        if age_hours > 24:
                            ...
                            cleaned += 1
                if cleaned:
                    logger.info(f"Temp cleanup: removed {cleaned} file(s)")
        except Exception as e:
...
def register_all(supervisor: Supervisor, send_cb):
    ...
    supervisor.add("temp_cleanup", _temp_cleanup_loop, args=(send_cb,))
