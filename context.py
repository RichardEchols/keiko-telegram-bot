...
def _atomic_json_write(path: Path, data) -> None:
    """Atomic JSON write with robust error handling.

    1. Ensures the parent directory exists
    2. Writes to a PID-unique temp file to avoid conflicts
    3. Sets explicit file permissions (owner rw)
    4. Renames temp -> target atomically
    5. Falls back to direct write on rename failure
    """
    try:
        path.parent.mkdir(parents=True, exist_ok=True)
    except Exception as e:
        logger.error(f"Cannot create directory {path.parent}: {e}")

    tmp = path.with_name(f".{path.stem}_{os.getpid()}.tmp")
    try:
        with open(tmp, "w") as f:
            json.dump(data, f, indent=2)
        # Ensure the file is readable/writable by the owner
        tmp.chmod(stat.S_IRUSR | stat.S_IWUSR | stat.S_IRGRP | stat.S_IROTH)
        tmp.replace(path)
        return
    except Exception as e:
        logger.error(f"Atomic write failed for {path} (tmp={tmp}): {e}")
    ...
    try:
        ...
        logger.info(f"Fallback direct write succeeded for {path}")
    except Exception as e2:
        logger.error(f"Fallback direct write also failed for {path}: {e2}")
...
def _save_history(history: List[Dict]) -> None:
    _atomic_json_write(CONVERSATION_HISTORY_FILE, history[-MAX_HISTORY:])
...
def save_correction(context: str, correction: str) -> None:
    ...
    _atomic_json_write(PREFERENCES_FILE, prefs)
