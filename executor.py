    ...
    try:
        ...
        try:
            while True:
                ...
                # Only send truly significant progress updates (deploys, builds, errors)
                if progress_callback:
                    progress = _parse_progress(line_text)
                    if progress and progress != last_update_content:
                        # Only send for significant events, with 30+ second gap
                        if _is_significant_progress(progress) and elapsed > 30:
...
def _is_significant_progress(progress: str) -> bool:
    """Check if a progress update is significant enough to send - VERY selective."""
    # Only send for deployments and errors - nothing else
    significant_prefixes = ["🚀", "⚠️", "❌"]
...
async def _build_context(project: Optional[Project] = None) -> str:
    ...
    # USER - Short summary only
    context_parts.append("""## About Richard (Quick Reference)
